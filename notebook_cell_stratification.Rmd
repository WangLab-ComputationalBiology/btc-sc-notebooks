...
seurat_cluster_composition <- seurat_cluster_composition %>%
  group_by(patient_id) %>%
  mutate(
    total_cell_per_patient = sum(n_cell)
  )

seurat_cluster_composition <- seurat_cluster_composition %>%
  ungroup() %>%
  mutate(
    total_cell_study = sum(total_cell_per_patient)
  )
...
seurat_cluster_composition <- seurat_cluster_composition %>%
  ...
  mutate(
    n_cell_per_cluster = sum(n_cell),
    percentage = (n_cell / n_cell_per_cluster) * 100
  )
...
seurat_cluster_composition <- seurat_cluster_composition %>%
  rowwise() %>%
  mutate(
    enrich_pvalue = phyper(
      q = n_cell - 1,
      m = total_cell_per_patient,
  ...
  tryCatch(
    {
        scven_result[[sample_prefix]] <- pipelineCNA(
          scevan_sample_counts[[sample_prefix]], organism = 'human', par_cores = n_threads, SUBCLONES = FALSE)
    }, 
