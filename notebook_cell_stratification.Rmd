...
params:
  project_name: 'Test'
  project_object: './dev/data/Test_cluster_object.RDS'
...
seurat_object <- readRDS(file = "./dev/data/Test_stratification_object.RDS")
...
infercnv_cluster_cnv <- infercnv_cluster_cnv %>%
  mutate(
    infercnv_label = if_else(
      avg_genomwide_cnv_proportion >= mean(avg_genomwide_cnv_proportion), "Tumor", "Normal" # Malignant / nonMalignant
    )
  )
...
consensus_cnv_score <- seurat_infercnv_object@meta.data %>%
  select(seurat_clusters, genomwide_cnv_proportion, n_affected_chromosomes, class, Cancer, Sort) %>%
...
consensus_cnv_score <- consensus_cnv_score %>%
  mutate(
    consensus_score = (genomwide_cnv_proportion + Cancer) ^ scevan_to_numeric
  )
...
if(auto_save) {
  ...
  saveRDS(seurat_object, file = paste0('./data/', project_name, '_stratification_object.RDS'))
