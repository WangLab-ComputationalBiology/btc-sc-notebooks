...
non_malignant_markers_index <- names(non_malignant_markers) %in% colnames(seurat_object@meta.data)
non_malignant_markers <- non_malignant_markers[non_malignant_markers_index]

if(length(non_malignant_markers) > 1) {
  
  for(cell_type in names(non_malignant_markers)) {
    seurat_object <- AddModuleScore(
    object = seurat_object,
    seed = random_seed,
    features = non_malignant_markers[cell_type],
    name = cell_type
    )
  }
  
  # Removing seeds on Module name
  column_indexes <- sapply(names(non_malignant_markers), function(column) {
...
seurat_cluster_composition <- seurat_cluster_composition %>%
  group_by(seurat_clusters, sctype, total_cell_study) %>%
  summarise(
...
# Boxplot
ggplot(seurat_cluster_composition, aes(x = seurat_clusters, y = percentage, fill = sctype)) +
  geom_col() +
  labs(x = "Seurat Clusters", y = "% of Cells", fill = "Cell annotation:") +
  theme_classic() +
  theme(
