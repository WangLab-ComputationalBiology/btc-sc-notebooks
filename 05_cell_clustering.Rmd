...
params:
  ...
  input_group_plot: 'source_name'
  input_batch_method: 'auto'
  n_threads: 20
  n_memory: 24
  thr_quantile: 'q01'
  thr_npc: 'auto'
...
for(sub_directory in c('data', 'figures', 'figures/clustered')) {
...
integration_method <- function(
    method = c('pca', 'mnn', 'rpca', 'cca', 'harmony', 'auto')) {
  ...
  if(method == 'auto') {
    if(file.exists('./data/batch_method.selected.txt')) {
      reduction_variable <- read_lines(file = "./data/batch_method.selected.txt")
    } 
  }
  
  if(method %in% names(seurat_object@reductions)) {
    reduction_variable <- input_batch_method
  } 
...
reduction_variable <- integration_method(
  method = input_batch_method
  )
...
seurat_cluster_density <- seurat_object@meta.data %>%
...
ggplot(seurat_cluster_density, aes(x = seurat_clusters, y = n_members)) +
  geom_col() +
...
seurat_cluster_density %>%
  tidyr::pivot_wider(names_from = seurat_clusters, values_from = n_members)
...
expression_counts <- GetAssay(object = seurat_object, assay = "RNA")
expression_counts <- expression_counts$counts
...
rogue_result <- rogue(expression_counts, labels = sample_clusters_members$clusters, samples = sample_clusters_members$Patient, platform = "UMI", span = 0.6)
...
umap_sample <- DimPlot(seurat_object, reduction = "umap", group.by = "sample_id", combine = FALSE)

if(auto_save) {

  ggsave(plot = umap_feature, filename = paste0('figures/clustered/UMAP_SAMPLE_', timestamp, '.pdf'))
...
umap_group <- DimPlot(seurat_object, reduction = "umap", group.by = "batch", combine = FALSE)

if(auto_save) {

  ggsave(plot = umap_feature, filename = paste0('figures/clustered/UMAP_BATCH_', timestamp, '.pdf'))
