...
kbet_calcution <- function(seurat_object, batch_method) {
    ...
    kbet_list <- sapply(unique(seurat_object$seurat_clusters), function(cluster_id) {
      ...
      kBET_tmp$summary$kBET.observed[1]
...
scpop_zscore <- calc_sumZscore(
  quality_metrics_summarise, 'batch')
...
lisi_calculation <- function(seurat_object, input_integration_method, batch_method) {
    ...
    lisi_result <- merge(
      lisi_result,
      seurat_object@meta.data[, c("barcode", "sctype")],
      by = "barcode"
    )
...
ggplot(lisi_harmony_result, aes(x = score)) +
  geom_density() + 
  labs(title = "Harmony") +
...
# Plotting LISI scores per cell
ggplot(lisi_best_result, aes(x = score)) +
  geom_density() + 
...
if(auto_save) {
  ...
  saveRDS(quality_metrics_summarise, file = paste0('data/', project_name, '_batch_table.RDS'))
