...
params:
  project_name: 'Test'
  project_object: './dev/data/Test_normalize_object.RDS'
...
seurat_cluster_density <- seurat_cluster_density %>%
  ...
  mutate(
    ...
    percentage = (n_cell / total_cell)*100
  )

#

ggplot(seurat_cluster_density, aes(x = seurat_clusters, y = percentage, fill = patient_id)) +
  geom_col() +
...
umap_sample <- DimPlot(seurat_object, reduction = "umap", group.by = "sample_id", raster = FALSE, combine = FALSE)
...
umap_group <- DimPlot(seurat_object, reduction = "umap", group.by = "batch", raster = FALSE, combine = FALSE)
