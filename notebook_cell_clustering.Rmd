...
reduction_variable <- integration_method(method = input_integration_method)
#reduction_variable <- paste0("integrated.", reduction_variable)
...
if(auto_save) {
  ...
  ggsave(plot = umap_sample, filename = paste0('figures/clustered/UMAP_SAMPLE.pdf'))
...
if(auto_save) {
  ...
  ggsave(plot = umap_group, filename = paste0('figures/clustered/UMAP_BATCH.pdf'))
    ...
    for(idx in 1:length(umap_group)) {
      ...
      umap_temp <-  umap_group[[idx]] + 
        theme(
          ...
          ggsave(plot = umap_temp, filename = paste0('figures/clustered/UMAP_GROUPED_', idx, '.pdf'))
...
if(auto_save) {
  ...
  saveRDS(seurat_object, file = paste0('./data/', project_name, '_', input_step_name, '_cluster_object.RDS'))
